...
namespace ForecastWizardApplication.Views
{
    ...
    public partial class FormPITagSelect : Form
    {
        ...
        public FormPITagSelect(SqlConnection connection,GetPITagUserControl uc)
        {
            InitializeComponent();
        ...
        private void OnClearDescInc_Click(object sender, EventArgs e)
        {
            textBoxDescInc.Text = "";
            QueryDatabase();
        }

        private void OnClearTagInc_Click(object sender, EventArgs e)
        {
        ...
        private void OnClearEngUnitsInc_Click(object sender, EventArgs e)
        {
            textBoxEngUnitsInc.Text = "";
            QueryDatabase();
        }

        private void OnClearDescExc_Click(object sender, EventArgs e)
        {
            textBoxDescExc.Text = "";
            QueryDatabase();
        }

        private void OnClearTagExc_Click(object sender, EventArgs e)
        {
        ...
        private void OnClearEngUnitsExc_Click(object sender, EventArgs e)
        {
        ...
        private void OnPointType_SelectedIndexChanged(object sender, EventArgs e)
        {
            QueryDatabase();
        }

        private void OnApplyFilters_Click(object sender, EventArgs e)
        {
            QueryDatabase();
        }
        ...
        private void QueryDatabase()
        {
            ...
            sqlString += " FROM PiSnapshot WHERE";
            ...
            try
            {
                ...
                /*
                numTags = adapter.Fill(dataSet, "pipoint");
                if (numTags < 1) return;
                */

                // Fill out the pisnapshot data set
                numTags = adapter.Fill(dataSet, "PiSnapshot");
                if (numTags < 1) return;
            }
            ...
            try
            {
                ...
                for (int i = 0; i < numTags; i++)
                {
                    ...
                    //DataRow dataRow1 = dataSet.Tables["pipoint"].Rows[i];
                    DataRow dataRow2 = dataSet.Tables["PiSnapshot"].Rows[i];

                    // Get the tag
                    string tag = (string)dataRow2["Tag"];
                    dataGridView1.Rows[i].Cells[0].Value = tag;

                    // Get the descriptor
                    string descriptor = (string)dataRow2["Descriptor"];
                    dataGridView1.Rows[i].Cells[1].Value = descriptor;

                    // Get the value
                    Object value;
                    if (dataRow2.IsNull("Value")) value = 0;
                    else value = (Object)dataRow2["Value"];
                    dataGridView1.Rows[i].Cells[2].Value = value.ToString();

                    // Get the engunits
                    string engunits = (string)dataRow2["EngUnits"];
                    dataGridView1.Rows[i].Cells[3].Value = engunits;

                    // Get the quality
                    string quality = "Bad";
                    int status = (int)dataRow2["Status"];
                    if (status == 0) quality = "Good";
                    dataGridView1.Rows[i].Cells[4].Value = quality;

                    // Get the timestamp
                    DateTime time = (DateTime)dataRow2["Time"];
        ...
        private void OnCellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 0)
            {
                ...
                selectedEngUnits = dataGridView1.Rows[e.RowIndex].Cells[3].Value.ToString();
                tagSelected = true;
