(function () {
    ...
    angular.module("seriveModule", [])
        .service('http', ['$http', function ($http) {
            ...
            this.getOptimizationGoals = function (isDayAhead, isCheck) {
                return $http({
                    method: "POST",
                    url: 'OptimizerGoals.aspx/GetOptimizationGoals',
                    dataType: 'json',
                    data: { isDayAhead: isDayAhead, isChecked: isCheck },
                    headers: { "Content-Type": "application/json" }
                }).success(function (data, responce) {
                })
            };
            ...
            /*----------------- Getting data for OptimizerGoals page for the chart series------------*/
            this.updateOptimizerGoals = function (values) {
                return $http({
                    method: "POST",
                    url: 'RTPUserDetails.asmx/UpdateOptimizerGoals',
                    data: "{" + "obj" + ":" + JSON.stringify(values) + "}",

                }).success(function (data, responce) {
                })
            };

            /*----------------- Getting data for forecast legend --------------------------------*/
            this.getForecastLegend = function () {
                return $http({
                    method: "POST",
                    url: 'Forecast.aspx/GetChartSettings',
                    data: {}
                }).success(function (data, responce) {
                })
            };

            /*----------------- Getting county name from the setting file --------------------------------*/
            this.getCountryName = function () {
                return $http({
                    method: "POST",
                    url: 'RTPUserDetails.asmx/GetCountyName',
                    data: {}
                }).success(function (data, responce) {
                })
            };
